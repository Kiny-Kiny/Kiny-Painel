...
while(Sair == False):
    ...
    print(f"{C}[{Y}Data de retorno das consultas por número{C}]: {C}[{G}05/05/2021{C}] {C}{Y}MOTIVO: ESTÁVAMOS PLANEJANDO RETORNAR COM A CONSULTA DE NÚMERO NO DIA 03/05, MAS SÓ IREMOS CONSEGUIR COMPRAR NO MESMO DIA A API, ENTÃO NÃO TERÍAMOS TEMPO DE CODAR{C}")
    time.sleep(0.2)
    print(f'''
    {C}[{Y}+{C}] Consultas:
